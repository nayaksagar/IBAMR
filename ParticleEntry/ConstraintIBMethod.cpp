...
/////////////////////////////// PUBLIC ///////////////////////////////////////
std::ofstream d_position_COM_file;
      ...
      d_rho_solid(d_no_structures, std::numeric_limits<double>::quiet_NaN())
{
    ...
    //print COM of all particles at all time in one single file
    if(!IBTK_MPI::getRank() && (d_timestep_counter % d_output_interval)==0){
        if (from_restart)
            d_position_COM_file.open(
                d_base_output_filename + "_COM_coordinates", std::fstream::app);
        else
            d_position_COM_file.open(
                d_base_output_filename + "_COM_coordinates", std::fstream::out);
    }
...
ConstraintIBMethod::calculateCOMandMOIOfStructures()
{
    ...
    if(!IBTK_MPI::getRank() && (d_timestep_counter % d_output_interval)==0){
       d_position_COM_file << d_FuRMoRP_current_time << std::endl;
       for(int struct_no = 0; struct_no < d_no_structures - 1; ++struct_no)//bug: -1 because of porous media
       {
         d_position_COM_file 
         << struct_no << "\t" << d_center_of_mass_current[struct_no][0] << "\t"
         << d_center_of_mass_current[struct_no][1] << std::endl;
       }
       d_position_COM_file << "\n" << std::endl;
    }
    
    /*// over-writable COM file for visualization
    if(!IBTK_MPI::getRank() && (d_timestep_counter % 50)==0){
    std::ofstream visCOM;
    visCOM.open("Dump--Cylinder//visualize_COM_file");
       //visCOM << d_FuRMoRP_current_time << std::endl;
       for(int struct_no = 0; struct_no < d_no_structures - 1; ++struct_no) //bug: -1 because of porous media
       {
         visCOM
         << d_center_of_mass_current[struct_no][0] << "\t"
         << d_center_of_mass_current[struct_no][1] << "\t" 
         << "0.03125" << std::endl;
       }
       visCOM << "\n" << std::endl;
    visCOM.close();
...
ConstraintIBMethod::calculateVolumeElement()
{
	...
	//d_structure_vol[struct_no] = M_PI*0.01*0.01;
